...
def room_temperature_intermediate(room, sensor_table):
    ...
    if temp_sensor_id:
        ...
        temp_sensor_ids = [f'{id}_temperature' for id in temp_sensor_id]
        or_conditions = ' OR '.join([f'"entity_id"=\'{id}\'' for id in temp_sensor_ids])

        query_room_temp = f'''SELECT MEAN("last_value") AS "room_temp" FROM (
            SELECT last("value") AS "last_value" FROM "ga_homeassistant_db"."autogen"."°C" 
            WHERE time > now() - 12h AND ({or_conditions}) 
            GROUP BY "entity_id"
        )'''

        print('query_room_temp', query_room_temp)
    
        room_temp = client_influx.query(query_room_temp)
        room_temp = pd.DataFrame(room_temp.get_points())
        print('room_temp', room_temp)

        return round(room_temp['room_temp'].values[0], 1) if not room_temp.empty else None
...
def room_temperature_last_ai(room, time):
    ...
    if temp_sensor_id:
        ...
        if temp_sensor_id:
            ...
            query_room_temp = f'''SELECT MEAN("last_value") AS "room_temp" FROM (
                SELECT last("value") AS "last_value" FROM "ga_homeassistant_db"."autogen"."°C" 
                WHERE time < '{formatted_time}' AND ({or_conditions}) 
                GROUP BY "entity_id"
            )'''
...
def heating_rate(room, radiator_id_list, temp_sensor_id):

    if radiator_id_list and temp_sensor_id:
    ...
    elif radiator_id_list:

        # Query to get last value for each room
        query = '''SELECT last("new_time_heat_up_1") AS "heating_rate"
        ...
        # Extract all heating rate values
        heating_rates = []

        # The ResultSet contains multiple series (one per group)
        for series in result.raw.get('series', []):
            # Each series has columns and values
            columns = series['columns']
            values = series['values']
            
            # Find the index of 'heating_rate' column
            if 'heating_rate' in columns:
                hr_index = columns.index('heating_rate')
                # Get the last value for this room
                if values and len(values) > 0:
                    last_value = values[0][hr_index]  # First row contains the last value
                    if last_value is not None:
                        heating_rates.append(last_value)
        
        # Alternative simpler approach using get_points() with measurement name
        # heating_rates = []
        # for point in result.get_points(measurement='hydraulic_balancing'):
        #     if 'heating_rate' in point and point['heating_rate'] is not None:
        #         heating_rates.append(point['heating_rate'])

        # Calculate the mean
        if heating_rates:
            average_heating_rate = sum(heating_rates) / len(heating_rates)
            print(f"Average heating rate: {average_heating_rate}")
            print(f"Number of rooms: {len(heating_rates)}")
            return average_heating_rate
        else:
            print("No data found for hydraulic balancing")
            return DEFAULT_HEATING_RATE  # Or return None/default value
    
    # If neither condition is met
    return DEFAULT_HEATING_RATE
...
def cooling_rate(room, radiator_id_list):
    ...
    #         print('The room average cooling rate', cooling_rate_df)
...
def heating_decision_ai(room, schedule, current_temp, is_heating, current_time, heating_rate, cooling_rate, max_valve_opening):
    ...
    if not system_info:
        ...
        return "idle", 100, next_time, temp_difference, current_target
...
def local_temperature_calibration(room, sub_system_mode, system_mode, valve_opening, temp_difference, new_target, mode_change, secondary_thu_offset):
    ...
    print('sub_system_mode', sub_system_mode, system_mode, valve_opening, temp_difference, new_target, mode_change)
    ...
    print('thermostat id', radiator_sensor_id)
    ...
    if temp_sensor_id:
        ...
        if room_temp_aq:
            room_temp_aq = room_temp_aq[0]['last']

    else: 
        room_temp_aq = room_temperature_intermediate(room, sensor_table)

    status = []

    if room_temp_aq is not None:
        ...
        for radiator in radiator_sensor_id:
            ...
            if shared_state['local_temperature'] is not None:
                ...
                if current_valve_opening is not None and (
                    ...
                    raw_temperature_offset = (room_temp_aq - radiator_temp) + current_temperature_offset
                    ...
                    if system_mode == 'MANUAL':
                        ...
                        temperature_offset = (room_temp_aq - radiator_temp) + current_temperature_offset + added_offset - early_stop
                        ...
                        temperature_offset = (room_temp_aq - radiator_temp) + current_temperature_offset + secondary_thu_offset
                        temperature_offset = round(temperature_offset, 1)
                        print(temperature_offset)
                        ...
                        temperature_offset = (room_temp_aq - radiator_temp) + current_temperature_offset + added_offset + overshoot_offset + secondary_thu_offset + extra_factor + occupancy_offset - early_stop_ai
                        temperature_offset = round(temperature_offset, 1)
                        print(temperature_offset)
                        ...
                        temperature_offset = (room_temp_aq - radiator_temp) + current_temperature_offset + overshoot_offset + secondary_thu_offset + extra_factor + occupancy_offset
                        temperature_offset = round(temperature_offset, 1)
                        print(temperature_offset)
...
def control_radiator(room, current_temp, room_radiators, parsed_schedule, current_time, heating_rate, cooling_rate, secondary_thu_offset):
    ...
    if actual_room_mode is not None:
        ...
        if actual_room_mode == 'MANUAL':
            ...
            print('new_sub_system_mode', new_sub_system_mode, 'valve_opening', valve_opening, 'next_time', next_time, 'temp_difference', temp_difference, 'new_target', new_target)
    ...
    for room in rooms_id:
        ...
        if room == 'flur':
            ...
            if radiators_id_list and not temp_sensor_id:
                ...
                current_room_temp = room_temperature_intermediate(room, sensor_table)
            ...
            if radiators_id_list and temp_sensor_id and not current_room_temp:
                ...
                room_heating_rate = heating_rate(room, radiators_id_list, temp_sensor_id)
